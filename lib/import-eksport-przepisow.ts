...
function bajtyDoBase64(bajty: Uint8Array): string {
  let wynik = '';
...
function base64DoBajtow(base64: string): Uint8Array {
...
/** Etykieta po polsku -> klucz w bazie. Przyjmuje też sam klucz, na wypadek gdyby ktoś wpisał "obiad" zamiast "Obiad". */
function odwrotnySlownik<T extends string>(slownik: Record<T, string>): Map<string, T> {
...
/** Rozdzielacz list w jednej komórce (kategorie, kuchnie, sprzęt). */
const ROZDZIELACZ = ';';

function podzielListe(tekst: string): string[] {
  return tekst
    .split(ROZDZIELACZ)
    .map((x) => x.trim())
    .filter((x) => x.length > 0);
...
function kluczNazwy(nazwa: string): string {
...
function tekstKomorki(wartosc: unknown): string {
...
function liczbaKomorki(wartosc: unknown): number | null {
...
function mapaNaglowkow(arkusz: Worksheet): Map<string, number> {
...
function komorka(wiersz: Row, naglowki: Map<string, number>, nazwaKolumny: string): unknown {
...
function dodajArkuszTabeli(wb: Workbook, nazwa: string, naglowki: readonly string[], wiersze: unknown[][]) {
